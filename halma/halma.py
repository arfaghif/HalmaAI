...
class Halma():
    ...
    def __init__(self, b_size=8, t_limit=60, c_player=Tile.P_RED, 
                 nodes_second=2000, max_node_generate=49):
        ...
        # self.ply_depth = 3
        self.nodes_second = 2000
        self.max_node_generate = 49
    ...
    def calculate_depth(self):
        # convert time limit into nodes limit
        n_limit = self.t_limit * self.nodes_second

        # get depth with logarithm
        self.ply_depth = math.floor(math.log(n_limit, self.max_node_generate))
    ...
    def minimax(self, depth, player_to_max, a=float("-inf"),
        ...
        if depth == 0 or self.find_winner() or self.time_counter == 1:
        ...
        for move in moves:
            for to in move["to"]:
                ...
                val, _, new_prunes, new_boards = self.minimax(depth - 1,
                    player_to_max, a, b, not maxing, prunes, boards)
                prunes = new_prunes
                boards = new_boards
    ...
    def execute_computer_move(self):
        ...
        if self.finish_execute:
            ...
            self.outline_tiles(None)  # Reset outlines
            move_from = self.board[self.best_move[0][0]][self.best_move[0][1]]
            move_to = self.board[self.best_move[1][0]][self.best_move[1][1]]
            self.move_piece(move_from, move_to)

            self.board_view.draw_tiles(board=self.board)  # Refresh the board

        else: # not (self.finish_execute)
            self.no_time_move_piece()
            print("not complete")
    ...
    def no_time_move_piece(self):

        # set board status that computer get time out
        self.board_view.set_status_color("#007F00" if
            self.current_player == Tile.P_RED else "#E50000")
        self.board_view.set_status(("Green" 
            if self.current_player == Tile.P_GREEN else "Red")
            + " player get time out, " + ("green's" if
            self.current_player == Tile.P_RED else "red's") + " turn...")
